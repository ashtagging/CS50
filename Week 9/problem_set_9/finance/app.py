...
def index():
    ...
    holdings = db.execute("SELECT symbol, SUM(shares) as total_shares FROM transactions WHERE user_id = ? GROUP BY symbol",
                          session["user_id"])
    ...
    for holding in holdings:
        ...
        stock["shares"] = holding["total_shares"]
...
def buy():
    ...
    if request.method == "POST":
        ...
        db.execute("INSERT INTO transactions (user_id, symbol, shares, price, timestamp) VALUES (?, ?, ?, ?, ?)",
                   session["user_id"], stock["symbol"], shares, stock["price"], datetime.datetime.now())
...
def history():
    ...
    transactions = db.execute("SELECT symbol, shares, price, timestamp FROM transactions WHERE user_id = ? ORDER BY timestamp DESC",
                              session["user_id"])
...
def sell():
    ...
    if request.method == "POST":
        ...
        # Retrieve user's current stock holding
        holding = db.execute("SELECT SUM(shares) as total_shares FROM transactions WHERE user_id = ? AND symbol = ?",
        ...
        # Retrieve user's stock symbols for select menu
